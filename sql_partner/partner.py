...
class res_company(orm.Model):
    ...
    _columns = {
        'sql_supplier_from_code': fields.char('SQL supplier from code >=', size=10, required=False, readonly=False),
        'sql_supplier_to_code': fields.char('SQL supplier from code <', size=10, required=False, readonly=False),
        'sql_customer_from_code': fields.char('SQL customer from code >=', size=10, required=False, readonly=False),
        'sql_customer_to_code': fields.char('SQL customer from code <', size=10, required=False, readonly=False),
        'sql_destination_from_code': fields.char('SQL destination from code >=', size=10, required=False, readonly=False),
        'sql_destination_to_code': fields.char('SQL destination from code <', size=10, required=False, readonly=False),
    }
...
class res_partner(orm.Model):
    ...
    def schedule_sql_partner_import(self, cr, uid, verbose_log_count=100, 
        ...
        try:
            ...
            if address_link:
                ...
                cursor = self.pool.get('micronaet.accounting').get_parent_partner(
                    cr, uid, context=context)
                if not cursor:
                    _logger.error("Unable to connect to parent for destination!")
                else:
                    for record in cursor:
                        destination_parents[record['CKY_CNT']] = record['CKY_CNT_CLI_FATT']
            ...
            for order, key_field, from_code, to_code, block in import_loop:
                ...
                for record in cursor:
                    ...
                    if verbose_log_count and i % verbose_log_count == 0:
                        _logger.info('Import %s: %s record imported / updated!' % (
                            block, i, ))                             
                        ...
                        if partner_ids:
                            try:
                                ...
                                try: # Remove vat for vat check problems:
                                    ...
                                    _logger.error('%s. Error updating partner [%s]: %s' % (
    ...
    _columns = {
        ...
        'sql_supplier_code':fields.char('SQL supplier code', size=10, required=False, readonly=False),
        'sql_customer_code':fields.char('SQL customer code', size=10, required=False, readonly=False),
        'sql_destination_code':fields.char('SQL destination code', size=10, required=False, readonly=False),
    }
